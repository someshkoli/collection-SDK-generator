...
const variable = require('postman-collection/lib/collection/variable');

const fs = require('fs'),
    fetchJson = require('./util').fetchJson,
    generator = require('../sdkgen'),
    collectionParser = require('./util').parseCollection,
    variableParser = require('./util').parseVariables,
    _ = require('lodash'),
...
SDKGenerator.prototype.setVariables = function (variables) {
    var self = this;
    fetchJson(variables, (err, variable) => {
        if (err) {
            return err;
        }
        self.variables = variable;
    });
    // variableParser(self.variableJson, (err, variable) => {
    //     if(!err){
    //         self.variables = variable;
    //     }
    //     else{
    //         console.log("error parsing postman variables");
    //     }
    // });
}
...
SDKGenerator.prototype.generate = function () {
    var self = this;
    var snippet = self.generator(self.collection, self.variables,null, function (err, snippet) {
